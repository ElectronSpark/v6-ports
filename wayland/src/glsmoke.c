...
static void frame_done(void *data, struct wl_callback *cb, uint32_t time)
{
    ...
    if (app->resize_seconds > 0 && app->last_resize_ns > 0 &&
        ...
        if (app->width == GL_WINDOW_W) {
            ...
            app->height = GL_WINDOW_H + TITLEBAR_H;
        }
                               ...
                               struct wl_array *states)
{
    ...
    if (width > 0 && height > 0 &&
        ...
        app->width = width;
        app->height = height;
        if (app->egl_window && app->egl_surface != EGL_NO_SURFACE)
            (void)recreate_egl_surface(app);
    }
}
...
static int init_wayland(struct app_state *app)
{
    ...
    xdg_toplevel_set_min_size(app->toplevel, GL_WINDOW_W / 2,
                              GL_WINDOW_H / 2 + TITLEBAR_H);
    if (init_gl(app) < 0)
        return -1;
