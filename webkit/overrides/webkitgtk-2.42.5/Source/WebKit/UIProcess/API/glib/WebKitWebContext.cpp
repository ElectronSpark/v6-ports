...
{
    /*
     * xv6's target runtime still has intentionally small pthread/libstdc++
     * one-time initialization support.  Avoid GLib's GOnce plus a C++ static
     * smart pointer here; the UI process initializes this on one thread.
     */
    static WebKitWebContext* webContext;
    if (!webContext) {
        webContext = WEBKIT_WEB_CONTEXT(g_object_new(WEBKIT_TYPE_WEB_CONTEXT, nullptr));
    }
    return webContext;
}
