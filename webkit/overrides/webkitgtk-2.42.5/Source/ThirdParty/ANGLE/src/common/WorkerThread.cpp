...
/*
 * xv6's current C++ runtime intentionally keeps std::thread minimal.  ANGLE
 * already has a single-threaded worker pool fallback, which matches the rest of
 * the WebKit xv6 profile where JSC worker/compiler concurrency is disabled.
 */
#undef ANGLE_STD_ASYNC_WORKERS
